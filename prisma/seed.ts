...
async function main() {
  const adminEmail = process.env.SEED_ADMIN_EMAIL ?? "admin@cnbs.local";
  const adminPassword = process.env.SEED_ADMIN_PASSWORD ?? "changeme123";
  const passwordHash = await bcrypt.hash(adminPassword, 12);

  const admin = await prisma.user.upsert({
    where: { email: adminEmail },
    update: { passwordHash, name: "Admin CNBS" },
    create: {
      email: adminEmail,
      name: "Admin CNBS",
      passwordHash,
  ...
  const classMilestones = [
    ...
    {
      slug: "workshop-found",
      titleFr: "Atelier CNBS trouvé",
      titleEn: "CNBS workshop secured",
...
Hull numbers 268, 269 and 270 are assigned. The "Old Stoves" blog is registered on the official Class Globe 5.80 website. Transparency, humor and honesty will guide this build documentation.`,
    ...
    },
    {
      ...
      excerptFr:
        "Congé Laurent, rôles définis, premières courbes Onshape et découverte du CNBS.",
      excerptEn:
        "Laurent's sabbatical, defined roles, first Onshape curves and discovering CNBS.",
...
La commande Allin est passée : 80 plaques de 10 mm okoumé marine (2500×1220 mm), livraison prévue fin 2025. Et puis la découverte du CNBS — une grange de 250 m² mise à disposition par les voisins de Marco, près de Saint-Valéry-sur-Somme. Le Chantier Naval de la Baie de Somme a trouvé son port d'attache.`,
...
The Allin order is placed: 80 sheets of 10 mm marine okoume (2500×1220 mm), delivery expected late 2025. Then comes the discovery of CNBS — a 250 m² barn offered by Marco's neighbors near Saint-Valéry-sur-Somme. The Chantier Naval de la Baie de Somme has found its home.`,
