...
- **Timeline / charge** : posts.workHours (heures produites, entier optionnel) ; milestones avec milestoneDate (début), endDate optionnel (fin période ; absent = jalon ponctuel), workloadForecast (heures prévues). Articles liés à un jalon comptent dans sa période si publishedAt ∈ [début, fin]. Métriques publiques : jours écoulés depuis début projet + somme workHours articles publiés.
